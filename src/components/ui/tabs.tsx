...
export function TabsList(props: Parameters<typeof KobalteTabs.List>[0] & { class?: string }) {
  ...
  return (
    <KobalteTabs.List
      class={cn(
        "inline-flex h-10 items-center justify-center rounded-lg bg-zinc-900/60 p-1 text-zinc-400 border border-zinc-800/80 backdrop-blur-sm",
        local.class
      )}
      {...others}
    />
...
export function TabsTrigger(props: Parameters<typeof KobalteTabs.Trigger>[0] & { class?: string }) {
  ...
  return (
    <KobalteTabs.Trigger
      class={cn(
        "inline-flex items-center justify-center whitespace-nowrap rounded-md px-3 py-1.5 text-xs sm:text-sm font-medium transition-all focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-zinc-400 disabled:pointer-events-none disabled:opacity-50 data-[selected]:bg-zinc-800 data-[selected]:text-zinc-100 data-[selected]:shadow-sm cursor-pointer",
        local.class
      )}
      {...others}
    />
